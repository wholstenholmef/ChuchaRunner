...
func add_score_multiplier():
	...
	tween.tween_property(get_node(score_multiplier_label), "scale", Vector2(1.0, 1.0), 0.2)
...
func reset():
	...
	get_node(score_multiplier_label).text = str(score_multiplier)
...
func _physics_process(delta):
	...
	#get_node(score_label).pivot_offset.x = get_node(score_label).size.x / 2
